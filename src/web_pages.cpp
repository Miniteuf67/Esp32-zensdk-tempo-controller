...
input,button{width:100%;padding:11px;margin:6px 0;font-size:16px}
button{background:#263746;color:#fff;border:0}
.net{background:#eef1f4;color:#222;text-align:left}
</style>
</head>
<body>
<div class="box">
  <h2>Zendure Tempo — première installation</h2>

  <button id="scan">Scanner le Wi-Fi</button>
  <div id="nets"></div>

  <form id="f">
    <input id="ssid" placeholder="SSID" required>
    <input id="wp" type="password" placeholder="Mot de passe Wi-Fi">
    <input id="ap" type="password" minlength="8" placeholder="Mot de passe administrateur (8 caractères min.)" required>
    <button>Enregistrer et redémarrer</button>
  </form>
...
q('#scan').onclick=async()=>{
  const a=await (await fetch('/api/setup/scan')).json();

  q('#nets').innerHTML=a.networks.map(n=>
    `<button type="button" class="net" data-s="${n.ssid.replace(/"/g,'&quot;')}">${n.ssid} • ${n.rssi} dBm</button>`
  ).join('');

  q('#nets').querySelectorAll('button').forEach(b=>{
    b.onclick=()=>q('#ssid').value=b.dataset.s;
  });
};
