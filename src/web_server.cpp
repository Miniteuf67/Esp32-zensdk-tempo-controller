...
static void sendWifiScan() {
  // En portail de secours, la liste initiale a déjà été scannée avant le SoftAP.
  // On tente ici un rafraîchissement. Si le scan en AP+STA échoue, on renvoie
  // le cache précédent au lieu d'une page vide.
  if (isSetupPortal()) {
    const String previous = setupWifiScanJson();
    refreshSetupWifiScanCache();
    String out = setupWifiScanJson();

    JsonDocument check;
    if (deserializeJson(check, out) || !bool(check["scan_ok"] | false)) {
      out = previous;
      Serial.println("[WIFI] Scan live indisponible -> utilisation du cache portail");
    }
  ...
  if (n > 0) {
    for (int i = 0; i < n && i < 25; ++i) {
      ...
      for (JsonObject existing : networks) {
        if (String(existing["ssid"] | "") == ssid) { duplicate = true; break; }
      }
...
void setupWebServer() {
  ...
  server.on("/api/setup/status", HTTP_GET, []() {
    ...
    JsonDocument d;
    d["saved_ssid"] = cfg.wifiSsid;
    d["has_saved_wifi"] = !cfg.wifiSsid.isEmpty();
    d["has_admin"] = cfg.adminPass.length() >= 8;
    d["ap_ssid"] = SETUP_AP_SSID;
  ...
  server.on("/api/setup/save", HTTP_POST, []() {
    ...
    const bool alreadyHasAdmin = cfg.adminPass.length() >= 8;
    if (ssid.isEmpty() || (!alreadyHasAdmin && adminPass.length() < 8) ||
        (!adminPass.isEmpty() && adminPass.length() < 8)) {
      server.send(
        400,
        "text/plain",
        alreadyHasAdmin
          ? "SSID requis. Si vous changez le mot de passe admin, utilisez au moins 8 caractères."
    ...
    if (!adminPass.isEmpty()) cfg.adminPass = adminPass;
    saveConfig();

    server.send(
      200,
      "text/plain",
      "Configuration enregistrée. Redémarrage automatique..."
    );

    // Laisser au navigateur le temps de recevoir la confirmation, puis quitter
    // proprement le mode AP/STA avant de repartir avec les identifiants NVS.
    Serial.printf("[WIFI] Nouvelle configuration sauvegardee: SSID=%s -> reboot automatique\n", cfg.wifiSsid.c_str());
    delay(1500);
